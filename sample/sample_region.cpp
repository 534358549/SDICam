...
int OSD_GetBitmapFromString(int ptSize, char *str, BITMAP_S *bitmap)
{
	...
	font = TTF_OpenFont("font.ttf", ptSize);
	if ( font == NULL )
	{
...
//	SDL_FreeSurface(temp);
	...
	temp = SDL_CreateRGBSurface(SDL_SWSURFACE, text->w, text->h, 16,
	...
	bitmap->u32Height = temp->h;
	memcpy(bitmap->pData, temp->pixels, temp->w * temp->h * 2);
	bitmap->enPixelFormat= PIXEL_FORMAT_RGB_1555;  

	SDL_FreeSurface(text);  
//	SDL_FreeSurface(temp);  为了测试，未释放内存
	TTF_CloseFont(font);
	TTF_Quit();
#endif
	return 0;
...
{
	int i, j;
	HI_U16 *pu16Temp = (HI_U16*)pstBitmap->pData;
...
//    printf("bmpInfo.bmiHeader.biWidth is %d\n", bmpInfo.bmiHeader.biWidth);
//    int i,j;
//    HI_U16 *pu16Temp;
//    pu16Temp = (HI_U16*)pstBitmap->pData;
    ...
    if (bFil)
    {
    	SAMPLE_RGN_SetTransperent(pstBitmap, u16FilColor);
		/*
        for (i=0; i<pstBitmap->u32Height; i++)
        {
            for (j=0; j<pstBitmap->u32Width; j++)
    ...
    HI_U32 u16FilColor, SIZE_S *pstSize, HI_U32 u32Stride, PIXEL_FORMAT_E enPixelFmt)
{
	...
	printf("CreateSurfaceByCanvas done\n");
	
	int ret = OSD_GetBitmapFromString(48, NULL, pstBitmap);
	printf("###### OSD_GetBitmapFromString returns %d, %dx%d\n", ret, pstBitmap->u32Width, pstBitmap->u32Height);
...
HI_S32 SAMPLE_RGN_StartVdec(VDEC_CHN VdecChn)
{
    ...
    stVdecAttr.stVdecVideoAttr.bTemporalMvpEnable = HI_FALSE;
...
HI_VOID *SAMPLE_RGN_VpssOsdReverse(void *pData)
{
	...
	memset(&stOsdChnAttr, 0, sizeof(RGN_CHN_ATTR_S));
...
HI_S32 SAMPLE_RGN_AddCoverAndOsdToVpss(HI_VOID)
{
    ...
    *************************************************/
    printf("step 1: create region and attach to vpss group\n");
    OverlayHandle    = 0;
    ...
    *************************************************/
#if 0
    printf("step 3: start vpss group and chn\n");
    s32VpssGrpNum = 1;
    ...
    *************************************************/
    printf("step 4: start vo dev and chn\n");
    ...
    *************************************************/
    printf("step 5: bind vdec and vpss\n");
    ...
    *************************************************/
    printf("step 6: bind vpss and vo\n");
    VoLayer = SAMPLE_RGN_GetVoLayer(VoDev);
    if(VoLayer < 0)
    {
        printf("SAMPLE_RGN_GetVoLayer failed! VoDev: %d.\n", VoDev);
    ...
    *************************************************/
    printf("step 7: load bitmap to region\n");
    ...
    handle 3 region used to invert color
    *************************************************/
    printf("create a thread to change regions layer and so on\n");
...
HI_S32 SAMPLE_RGN_AddCoverOsdAndLineToVpss(HI_VOID)
{
	...
	printf("step 0: region system init\n");
    s32Ret = SAMPLE_RGN_SYS_Init();
    if(HI_SUCCESS != s32Ret)
    {
        printf("SAMPLE_RGN_SYS_Init failed! s32Ret: 0x%x.\n", s32Ret);
    ...
    *************************************************/
    ...
    layer, position, alpha and other display attribute
    *************************************************/
    printf("step 8: create a thread to change regions layer, position, alpha and other display attribute\n");
