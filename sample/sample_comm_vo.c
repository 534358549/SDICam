...
// set a single channel to the output device
// added by wilson at 2017.12.16
HI_S32 SAMPLE_COMM_VO_SetSingleChn(VO_LAYER VoLayer, int chn)
{
...
//	printf("############# stChnAttr: %d x %d\n", stChnAttr.stRect.u32Width, stChnAttr.stRect.u32Height);

	s32Ret = HI_MPI_VO_SetChnAttr(VoLayer, chn, &stChnAttr);
	if (s32Ret != HI_SUCCESS)
    {
	...
	s32Ret = HI_MPI_VO_EnableChn(VoLayer, chn);
    if (s32Ret != HI_SUCCESS)
    {
        SAMPLE_PRT("failed with %#x!\n", s32Ret);
        return HI_FAILURE;
